...
df_train, df_val = generators.amateur_train_val_split('G:/AMATEUR/segmentation/22FEV2019/GEN_segmentation/', number_elements=None)

dim_image = (256, 256, 3)
model = unet(input_size=dim_image)

train_generator = AmateurDataFrameDataGenerator(df_train, batch_size=4, img_w=dim_image[0], img_h=dim_image[1])
val_generator = AmateurDataFrameDataGenerator(df_val, batch_size=4, img_w=dim_image[0], img_h=dim_image[1])
